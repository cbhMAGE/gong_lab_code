...
class Tool:
    ...
    def gen_cartesian_traj(self, target_config: List, prefix: str, num=50, dis=0.2, move_x=False, start=0.0) -> List[RobotConfig]:
        ...
        for i in np.linspace(start, dis, num=num):
            mid_tf = target_tf * Transform.build_from_euler_point(Euler(), Point(0, 0, -i))
            if move_x:
                mid_tf = Transform.build_from_euler_point(Euler(), Point(-i, 0, 0)) * target_tf
...
def split_traj(traj: moveit_msgs.msg.RobotTrajectory):
    l_traj = moveit_msgs.msg.RobotTrajectory()
    r_traj = moveit_msgs.msg.RobotTrajectory()
    for p in traj.joint_trajectory.points:
        p: trajectory_msgs.msg.JointTrajectoryPoint = p
        l_p = trajectory_msgs.msg.JointTrajectoryPoint()
        r_p = trajectory_msgs.msg.JointTrajectoryPoint()
        l_p.time_from_start = deepcopy(p.time_from_start)
        r_p.time_from_start = deepcopy(p.time_from_start)
        for i in range(len(traj.joint_trajectory.joint_names)):
            if 'left' in traj.joint_trajectory.joint_names[i]:
                l_p.positions.append(p.positions[i])
            else:
                r_p.positions.append(p.positions[i])
        l_traj.joint_trajectory.points.append(l_p)
        r_traj.joint_trajectory.points.append(r_p)
    for i in range(len(traj.joint_trajectory.joint_names)):
        j_name = traj.joint_trajectory.joint_names[i]
        if 'left' in j_name:
            l_traj.joint_trajectory.joint_names.append(j_name)
        else:
            r_traj.joint_trajectory.joint_names.append(j_name)
...
def main():
    ...
    # 11
    targets = [
        ([-122.4,69.8,-85.7,177.6,108.3,70.7,], [135.7,128.2,80.6,-6.1,-125.3,-213.8,]),
    ...
    r_configs = tool.gen_cartesian_traj(r, "right_", c_num, dis=0.2)
    dual.set_start_state(Config.dual_home_config.to_robot_states())
    dual.set_joint_value_target((r_configs[-1] + Config.left_home_config).to_dict())
    s1, p1, t1, e1 = dual.plan()
    if not s1:
        rospy.logerr("right failed")
        return
    r_sp_configs = tool.gen_cartesian_traj(r, "right_", c_num, move_x=True)

    l_configs = tool.gen_cartesian_traj(l, "left_", c_num)
    dual.set_start_state((r_sp_configs[-1] + Config.left_home_config).to_robot_states())
    dual.set_joint_value_target((r_sp_configs[-1] + l_configs[-1]).to_dict())
    s2, p2, t2, e2 = dual.plan()
    if not s2:
        rospy.logerr("left failed")
        return
    r_ff_configs = tool.gen_cartesian_traj(rad2deg(r_sp_configs[-1].to_array()), "right_", c_num, dis=0.2)

    dual.set_start_state((r_ff_configs[-1] + l_configs[-1]).to_robot_states())
    dual.set_joint_value_target((Config.right_home_config + Config.left_place_config).to_dict())
    s3, p3, t3, e3 = dual.plan()
    if not s3:
        rospy.logerr("left place failed")
        return

    rc.dual_move([p0])
    rc.open_hand("left_")
    rc.close_hand("right_")
    r_c2 = [
        c + Config.left_home_config for c in r_configs
    ]
    rc.dual_move([p1, configs_to_robot_trajectory(r_c2[::-1])])
    sp_c = [
        c + Config.left_home_config for c in r_sp_configs
    ]
    rc.dual_move([configs_to_robot_trajectory(sp_c)])
    l_c2 = [
        c + r_sp_configs[-1] for c in l_configs
    ]
    rc.dual_move([p2, configs_to_robot_trajectory(l_c2[::-1])])
    rc.close_hand("left_")
    # sp_c2 = [
    #     c + l_configs[0] for c in r_sp_configs
    # ]
    # rc.dual_move([configs_to_robot_trajectory(sp_c2[::-1])])
    rc_c = [
        l_configs[i] + r_ff_configs[i] for i in range(c_num)
    ]
    rc.dual_move([configs_to_robot_trajectory(rc_c), p3])
    rc.open_hand("left_")
