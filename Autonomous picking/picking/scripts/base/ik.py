...
import numpy as np
from tools import deg2rad, rad2deg, wrap2pi, get_config_from_state, moveit_error_code_to_string
from dto import Transform
from typing import List


class AnalyticalIK:
    def __init__(self, robot_type='zu7_v2'):
        """
        d1 a2 a3 d5
                    d6
                    d4
        ...
        for _i in range(6):
            for _j in range(2):
                self.bounds[_i][_j] = deg2rad(self.bounds[_i][_j]) * 0.85
    ...
    def solve(self, tcp2base: Transform, seed=None, tcp2end=Transform(), seed_threshold_deg=12) -> (List, float):
        ...
        if seed:
            ...
            for sol in f2:
                ...
                for _i in range(6):
                    ...
                    if _d > deg2rad(seed_threshold_deg):
                        found = False
...
class NumericRosIK:
    def __init__(self, prefix):
        self.ik = rospy.ServiceProxy('/compute_ik', moveit_msgs.srv.GetPositionIK)
        self.req = moveit_msgs.srv.GetPositionIKRequest()
        self.prefix = prefix
        self.req.ik_request.group_name = prefix + "arm"
        self.req.ik_request.robot_state.joint_state.name = ["%sjoint_%d" % (prefix, i) for i in range(1, 7)]
        self.req.ik_request.timeout = rospy.Duration.from_sec(0.2)

    def solve(self, end2base: Transform, seed=(0, 0, 0, 0, 0, 0)) -> list:
        self.req.ik_request.robot_state.joint_state.position = seed
        self.req.ik_request.pose_stamped = end2base.to_msg_pose_stamped(frame_id='right_base_link')
        resp: moveit_msgs.srv.GetPositionIKResponse = self.ik(self.req)
        # print(moveit_error_code_to_string(resp.error_code))
        return get_config_from_state([self.prefix], resp.solution)
...
if __name__ == '__main__':
    ...
    ik = AnalyticalIK(robot_type='zu7_v2')

    ik.set_joints_with_real_j_vals(deg2rad([90, 30, 30, 70, 30, 50]))

    # ik.set_joints_with_real_j_vals(deg2rad([0, 0, 0, 0, 0, 0]))

    tf = ik.forward_kinematics(6, 0)
    print(tf)
    # for _row in tf:
    #     print(",".join(map(str, _row)), end=',\n')

    print("sxyz", tf.to_euler())
    print("qua", tf.to_qua())
    import time

    ans = ik.solve(tf.offset(1, 1, 1))
    print(len(ans))

    print("ans")
    for _row in ans:
        print(",".join(map(lambda x: "%8.2f" % (x / pi * 180.0), _row)), end=',\n')

    ik2 = NumericRosIK(prefix='right_')

    t = timeit.default_timer()
    for i in range(300):
        ans2 = ik2.solve(tf, deg2rad([90.00, 57.39, -30.00, 102.61, 30.00, -310.00]))
        if not ans2:
            print("failed")
        # print("ans2", i)
        # print(",".join(map(lambda x: "%8.2f" % (x / pi * 180.0), ans2)), end=',\n')
