...
本项目致力于构建一个融合强化学习（Reinforcement Learning, RL）与视觉语言行动（Vision-Language-Action, VLA）模型的机器人智能决策系统。该代码仓库集成了多模态感知（视觉 + 语言）与强化学习策略优化能力，旨在提升机器人在真实复杂环境中的任务泛化能力与智能行为决策效率。

我们采用主流的多模态模型（如SigLIP、QwenVL等）进行视觉-语言理解，并结合强化学习算法（如SAC、DPO、GRPO、Diffusion Policy等）实现高效动作生成，支持以下功能：

🧠 基于图像与语言指令的任务识别与环境理解

🎯 融合多模态信息的策略学习与动作生成

🤖 支持仿真环境与真实机器人平台的任务部署与评估

🧪 覆盖多个标准评估基准（如RLBench、CALVIN、ManiSkill、VLABench等）

本仓库适用于开展多模态任务规划、端到端控制、模仿学习与强化学习结合等研究，广泛服务于机器人抓取、操作、导航等领域，尤其面向开放世界任务与复杂指令理解的智能体系统研发与训练。
