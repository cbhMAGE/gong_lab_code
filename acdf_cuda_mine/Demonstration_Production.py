...
class ACDFBuffer:
    ...
    def store(self, obs, act, rew, val, logp, done=0, std=0.):
        """
    ...
    def Demon_get(self):
        ...
        adv_mean, adv_std = mpi_statistics_scalar(self.adv_buf)
        self.adv_buf = (self.adv_buf - adv_mean) / adv_std
        data = dict(obs_buf=self.obs_buf, act_buf=self.act_buf, rew_buf=self.rew_buf,
                     logp_buf=self.logp_buf, done_buf=self.done_buf)
        #return {k: torch.as_tensor(v, dtype=torch.float32, device=device) for k,v in data.items()}
        return {k: v for k,v in data.items()}


def acdf(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
         steps_per_epoch=4000, epochs=50, Demon_epochs=50, pi_epochs=20, vf_epochs=20, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
    ...
    o, ep_ret, ep_len = env.reset(), 0, 0
    Demon_buffer_size = int(local_steps_per_epoch/4)
    Demon_buf = ACDFBuffer(obs_dim, act_dim, Demon_buffer_size, gamma, lam)
    SAVE_Demonstration = open('Jaka50.pickle', "wb")



    # Replay and store the demonstration data
    episode = [None] * Demon_epochs
    data_to_save = {}
    counter = 0

    for epoch in range(Demon_epochs):
        for t in range(Demon_buffer_size):
            a, v, logp = ac.step(torch.as_tensor(o, dtype=torch.float32, device=device))
            ...
            # save and log
            Demon_buf.store(o, a, r, v, logp, done=d)
            logger.store(VVals=v)
            ...
            timeout = ep_len == max_ep_len
            terminal = d or timeout
            epoch_ended = t==Demon_buffer_size-1
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    ...
                    v = 0
                Demon_buf.finish_path(v)
                if terminal:
                    # only save EpRet / EpLen if trajectory finished
                    logger.store(EpRet=ep_ret, EpLen=ep_len)
                o, ep_ret, ep_len = env.reset(), 0, 0

                #Key_turn = str(counter)
                #data_to_save[counter] = copy.deepcopy(Demon_buf.Demon_get())
                #counter += 1

        #Key_turn = str(epoch)
        data_to_save[epoch] = copy.deepcopy(Demon_buf.Demon_get())
        #print('Episode',epoch, data_to_save[Key_turn])
    print('LENGTH:', len(data_to_save))
    #data_to_save = dict(zip(range(0, Demon_epochs), episode))
    pickle.dump(data_to_save,SAVE_Demonstration)
        #print(data_to_save)
    SAVE_Demonstration.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Demon-epochs', type=int, default=100)
    parser.add_argument('--exp_name', type=str, default='Demonstration_Producer_Jaka')
    parser.add_argument('--demo-file', type=str, default='Ant50epoch.pickle')
    ...
    acdf(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
         ...
         Demon_epochs=args.Demon_epochs,pi_epochs=args.pi_epochs, vf_epochs=args.vf_epochs,
