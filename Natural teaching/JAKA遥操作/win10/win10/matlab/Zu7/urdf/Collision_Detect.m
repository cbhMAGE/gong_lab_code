...
global JAKAzu7Tree
step = 50/1000; % step needs to be decide
v_max = pi; a_max = 2*pi; q_step_max = pi / 6;
v_safe = 5; a_safe = 2;
...
% Suppose all joints slow down with max acceleration
q_predict = zeros(6,1);
for idx = 1:6
    q_predict(idx) = q_tar(idx) + v_next(idx) * dq_abs(idx) / (2*a_max);
    disp(q_predict);
end
% q_predict = q_tar + v_next * dq_max / (2*a_max);
isConfigInCollision = checkCollision(JAKAzu7Tree,q_predict,'Exhaustive','on');
% disp(isConfigInCollision)
