...
def plot_data(data_to_plot, environments):
    ...
    for i, ax in enumerate(axs):
     ...
     #   while ppo[z] < 0.2 * max_PPO:
      #    z+=1
        # print(z)
     #   our = data_to_plot["Pre-trained"][environments[i]][1]
     #   mm = 0
     ...
     #   print(environments[i], mm / z)
        ...
        ax.legend(ncol=1, fontsize=13, loc='lower right')
...
def make_plots(root_path, environments, legend=None, xaxis=None, values=None, count=False,
    ...
    data_to_plot['PPO'] = data_with_suffix(root_path + 'InmoovGymEnvPPO/', suffix='', environments=environments)
    # data_to_plot['Ours'] = data_with_suffix(root_path, 'ACDF_pi20_vf20', environments)
    data_to_plot['Pre-trained'] = data_with_suffix(root_path + 'InmoovGymEnvACDF_pi20_vf20/', suffix='', environments=environments)
