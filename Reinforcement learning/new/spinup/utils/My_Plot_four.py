...
def plot_data(data_to_plot, environments):
    ...
    for i, ax in enumerate(axs):
        ...
        with_demons = data_to_plot['WithDemon'][environments[i]][1]
        no_demons = data_to_plot["NoDemon"][environments[i]][1]
        max_no_demons = max(no_demons)
        min_no_demons = min(no_demons)
        max_division = max_no_demons - min_no_demons
        z = 0
        while with_demons[z]-min(with_demons) < 0.2 * max_division:
          z+=1
        #print(z)
        
        mm = 0
        while no_demons[mm]-min(no_demons) < 0.2 * max_division:
          mm += 1
        #print(mm)
        print(environments[i], z / mm)
        ...
        if((environments[i] == 'Swimmer') or (environments[i] == 'Humanoid')): ax.legend(["With Demonstration", "PPO"], ncol=1, fontsize=15, loc='lower right')
        else: ax.legend(["With Demonstration", "PPO"], ncol=1, fontsize=15, loc='upper left')
...
def main():
    ...
    environments = ['Hopper', 'Ant', 'Humanoid', 'Swimmer']
    #environments = ['InMoov', 'Jaka']
    #environments = ['Ant_CUT_Off', 'Hopper_CUT_Off']
    environments.sort()
