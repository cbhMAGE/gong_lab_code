...
def data_with_suffix(root, suffix, environments, axis='TotalEnvInteracts'):
    if root[-1] != '/':
        root += "/"
    logdirs = [root+suffix for env in environments]
...
def plot_data(data_to_plot, environments):
    ...
    #axs = [plt.subplot(141 ), plt.subplot(142 ), plt.subplot(143),
     #plt.subplot(144 )]
    axs = [plt.subplot(111 )]
...
def make_plots(root_path, environments, legend=None, xaxis=None, values=None, count=False,
    ...
    data_to_plot['1'] = data_with_suffix(root_path, '1', environments)
    data_to_plot['2'] = data_with_suffix(root_path, '1', environments)
    data_to_plot['3'] = data_with_suffix(root_path, '1', environments)
    data_to_plot['4'] = data_with_suffix(root_path, '1', environments)
    #data_to_plot['5'] = data_with_suffix(root_path, '1', environments)
    #data_to_plot['Ours'] = data_with_suffix(root_path, 'Humanoid_2022_11_7_ours', environments)
    #data_to_plot['PPO'] = data_with_suffix(root_path, 'Swimmer_ppo_2022_11_6_222', environments)
    #data_to_plot['NoDemon'] = data_with_suffix(root_path + 'PPO_No_Demons/', suffix='', environments=environments)
    #data_to_plot['PPO'] = data_with_suffix(root_path + 'PPO', suffix='', environments=environments)
    #data_to_plot['Pre-Trained'] = data_with_suffix(root_path + 'ACfD', suffix='',environments=environments)
    #data_to_plot['While Training'] = data_with_suffix(root_path + 'Birch', suffix='',environments=environments)
    ...
    #algo1_name = 'PPO'
    #algo2_name = 'Integrated'
    #env_name = 'InMoov'
    #sum = []
    #policy_regret = np.sum(data_to_plot[algo1_name][env_name][1]-data_to_plot[algo2_name][env_name][1])
    #std = np.sum(data_to_plot[algo1_name][env_name][2]-data_to_plot[algo2_name][env_name][2])

    #print("policy_regret", policy_regret)
    #print("policy_regret_std", std)
...
def main():
    ...
    #environments = ['Hopper', 'Ant', 'Swimmer', 'InMoov']
    environments = ['InMoov']
    environments.sort()
    ...
    #root_path="/home/gong112/service_backup/work/new/data_plot/Automatica/"
    root_path="/home/gong112/service_backup/work/RL-InmoovRobot/data/"
