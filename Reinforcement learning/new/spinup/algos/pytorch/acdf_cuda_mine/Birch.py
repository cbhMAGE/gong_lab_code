...
import time
import sys
sys.path.append("/home/cbhmage/service_backup/work/RL-InmoovRobot/")
from environments.registry import *
import copy
from sklearn.cluster import Birch
import spinup2.algos.pytorch.acdf_cuda.core as core
from spinup2.algos.pytorch.acdf_cuda import core
from spinup2.utils.logx import EpochLogger
from spinup2.utils.mpi_pytorch_cuda import setup_pytorch_for_mpi, sync_params, mpi_avg_grads
from spinup2.utils.mpi_tools_cuda import mpi_fork, mpi_avg, proc_id, mpi_statistics_scalar, num_procs
from sklearn.datasets import make_blobs
from spinup2.algos.pytorch.acdf_cuda_mine.demo_env import DemoGymEnv
...
def acdf(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            #if epoch == 0 or epoch == 100 or epoch == 200 or epoch == 300 or epoch == 400 or epoch ==490:
            #    env.render()
        ...
        # 构造Obs聚类器
        num_of_clusters_obs = int(Obs_data_normalised.shape[1] / 6) #+ 3

        #Obs_data_normalised, y = make_blobs(n_samples=10000, n_features=8, centers=[[-1, -1], [0, 0], [1, 1], [2, 2]],
        #                  cluster_std=[0.4, 0.3, 0.2, 0.2], random_state=9)
        estimator_obs = Birch(n_clusters=num_of_clusters_obs, threshold=0.05, branching_factor=10)  # 构造聚类器
        #print('shapecheck', np.shape(Obs_data_normalised))
        estimator_obs.fit_predict(Obs_data_normalised)  # 聚类
        ...
        for j in range(0, num_of_clusters_obs):
            ...
            num_of_clusters_act = int(Act_data_normalised.shape[1] / 2) #+ 2
            estimator_act.append(Birch(n_clusters=num_of_clusters_act, threshold=0.05, branching_factor=10))  # 构造聚类器
        ...
        for j in range(int(local_steps_per_epoch * demon_ratio)):
            ...
            #print('predict', estimator_obs.predict([o_norm, ]))
            #print('cluster', estimator_obs.subcluster_centers_.T)
            #print('cluster_TT', estimator_obs.subcluster_centers_)
            #print('SHAPE_act:::',Act_data_normalised.shape)
            #print('SHAPE_obs:::',Obs_data_normalised.shape)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--exp_name', type=str, default='Inmoov_2022_11_20')
    parser.add_argument('--exp_name', type=str, default='Swimmer_2022_11_20')
    #parser.add_argument('--exp_name', type=str, default='Humanoid_2022_11_7_ours2')
    # parser.add_argument('--demo-file', type=str, default='./Demonstration_Data/InmoovGymEnv100.pickle')
    #parser.add_argument('--demo-file', type=str, default='./Demonstration_Data/Humanoid100.pickle')
    parser.add_argument('--demo-file', type=str, default='./Demonstration_Data/Swimmer100.pickle')
...
#    for args.seed in range(0, 5):
    for args.seed in range(3, 5):
