...
from spinup.algos.pytorch.abc.acdf import device
class trainset(Dataset):
    def __init__(self, demo_path, size=None):
        self.demo_path = demo_path
        self.load_data()
        return

    def load_data(self):
        with open(self.demo_path, 'rb') as fp:
        ...
        for k in demo_data.keys():
            eps_length = len(demo_data[k][o_key]) - 1
            eps_data = demo_data[k]
            for i in range(eps_length):
                counter += 1
                all_obs.append(eps_data[o_key][i])
                all_act.append(eps_data[a_key][i+1])
        print("The size of all demonstration data: {}".format(counter))
        all_obs = np.array(all_obs)
        all_act = np.array(all_act)
        self.all_obs = torch.tensor(all_obs, dtype=torch.float32, device=device)
        self.all_act = torch.tensor(all_act, dtype=torch.float32, device=device)

    def __getitem__(self, index):
        obs = self.all_obs[index]
        act = self.all_act[index]
        return obs, act
...
if __name__ == "__main__":
    ...
    # b= env.step(1)
    # b= env.step(2)
    trainingset = trainset('data/Ant/Ant1000.pickle')
